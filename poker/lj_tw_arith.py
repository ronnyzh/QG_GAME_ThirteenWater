...
ALLSETTING = False
###配置表
IsHas_FIVE_HEADS = ALLSETTING  # 五同
##特殊牌型
IsHas_SEVEN_HEADS = ALLSETTING  # 七同
IsHas_TWELVE_ROYALTY = ALLSETTING  # 十二皇族
IsHas_THREE_POINTS = ALLSETTING  # 三分天下
IsHas_SIX_HEADS = ALLSETTING  # 六同
IsHas_ALL_BIG = ALLSETTING  # 全大
IsHas_ALL_SMALL = ALLSETTING  # 全小
IsHas_ONE_COLOR = ALLSETTING  # 凑一色
IsHas_FOUR_TRIPLET = ALLSETTING  # 四套三条
IsHas_FIVE_PAIRS_A_TRIPLET = ALLSETTING  # 五对三条

type2Texts = {
    INVALID_TYPE: "无效牌".decode('utf-8'),
    # OOLONG: "乌龙".decode('utf-8'),
    OOLONG: "散牌".decode('utf-8'),
    ...
    SUPREME_QINGLONG: "同花顺一条龙".decode('utf-8'),
}
...
if IsHas_FIVE_HEADS:
    type2GetCardsFunc[FIVE_HEADS] = _getCardData_FH
...
def getSpecialType(cards, isCommonPlay=True):
    ...
    if isCommonPlay:
        ...
        if valCounts.count(4) + valCounts.count(5) == 3 and IsHas_THREE_POINTS:
            oneVal = count2vals[1][0]
            cards = dealOneSort(cards, oneVal)
            return SpecialType(cards, THREE_POINTS)
        if maxCount == 6 and IsHas_SIX_HEADS:
            mostVal = count2vals[maxCount][0]
            cards = dealCountSort(cards, mostVal)
            return SpecialType(cards, SIX_HEADS)
        # 全大, 全小, 凑一色, 四套三条, 五对三条
        if minVal >= 8 and IsHas_ALL_BIG:
            return SpecialType(cards, ALL_BIG)
        if maxVal <= 9 and IsHas_ALL_SMALL:
            return SpecialType(cards, ALL_SMALL)
        if colorSet <= set(BLACK_COLOR) or colorSet <= set(RED_COLOR) and IsHas_ONE_COLOR:
            return SpecialType(cards, ONE_COLOR)
        if valCounts.count(3) + valCounts.count(4) == 4 and IsHas_FOUR_TRIPLET:
            oneVal = count2vals[1][0]
            cards = dealOneSort(cards, oneVal)
            return SpecialType(cards, FOUR_TRIPLET)
        if valCounts.count(2) == valCounts.count(3) + 4 == 5 and IsHas_FIVE_PAIRS_A_TRIPLET:
            mostVal = count2vals[maxCount][0]
            cards = dealCountSort(cards, mostVal)
            return SpecialType(cards, FIVE_PAIRS_A_TRIPLET)
    # 六对半, 三顺子, 三同花
    # if valCounts.count(2) == 6:
    if valCounts.count(1) == 1 and valCounts.count(3) == 0:
        oneVal = count2vals[1][0]
        cards = dealOneSort(cards, oneVal)
        return SpecialType(cards, SIX_PAIRS)
    elif valCounts.count(3) == 1 and valCounts.count(1) == 0:
        oneVal = count2vals[3][0]
        cards = dealOneSort(cards, oneVal)
        return SpecialType(cards, SIX_PAIRS)
...
def CardsModelFactory(cards):
    ...
    if _len == 3:
        ...
        # [5]
        if lenValSet == 1 and IsHas_FIVE_HEADS:
...
def _getBestChoice(allVals, jokers, color2vals):
    ...
    maxScore = 0
    # 五同
    if allFiveList and IsHas_FIVE_HEADS:
        tailScore = 9
        isTrue = True
...
def updatePierList(totalScore, tailPier, middlePier, headPier, maxScore, curPierList):
    if totalScore >= maxScore and tailPier and middlePier and headPier and tailPier[0] >= middlePier[0] >= headPier[0]:
...
class SpecialType(CardsModel):
    ...
    def isEq(self, m):
        return (self.type in FIRST_EQ and m.type in FIRST_EQ) or (self.type in SECONED_EQ and m.type in SECONED_EQ)
...
class StraightFlush(CardsModel):
    def __gt__(self, m):
        if self.type > m.type:
            return True
        if self.type == m.type:
            myVal, _, _, = getCardData(self.cards)
            targetVal, _, _, = getCardData(m.cards)
            # 牌值全部一样
            if set(myVal) == set(targetVal):
                print('同花顺 my card %s' % self.cards)
                print('同花顺 target card %s' % m.cards)
                color = {'d': 4, 'c': 3, 'b': 2, 'a': 1}
                return color.get(self.cards[0][1]) > color.get(m.cards[0][1])
            selfVal = self.vals[0]
            otherVal = m.vals[0]
...
class Flush(CardsModel):
    def __gt__(self, m):
        if self.type > m.type:
            return True
        if self.type == m.type:
            if set(self.vals) == set(m.vals):
                print('同花 my card %s' % self.cards)
                print('同花 target card %s' % m.cards)
                color = {'d': 4, 'c': 3, 'b': 2, 'a': 1}
                return color.get(self.cards[0][1]) > color.get(m.cards[0][1])
...
if __name__ == '__main__':
    # curCards = ['2a', '2b', 'Ad', 'Kb', 'Ka', '5a', '5d', '5c', '5b', '5a', '3d', '3c', '3a']
    curCards = ['3d','3b', 'Qa', 'Kc','Kb','9c','8c','5b','Ad','Ac','Ab','Aa','2b']
    ...
    ins1_1 = rsult[1]
    ins1_2 = rsult[2]
    print ins1_0.__repr__(), ins1_1.__repr__(), ins1_2.__repr__()

    curCards = ['Ka','Qc','Td','4d','4c','4b','7c','5d','Jd','Jc','Jd','2d','2a']
    curCards = cardSort(curCards)
    rsult = getDefaultType(curCards)
    rsult.reverse()
    ins2_0 = rsult[0]
    ins2_1 = rsult[1]
    ins2_2 = rsult[2]
    print ins2_0.__repr__(), ins2_1.__repr__(), ins2_2.__repr__()

    print '%s %s %s '%(ins1_2.__repr__()  ,  '>' if ins1_2>ins2_2 else '<'  ,ins2_2.__repr__())

    card1 = CardsModelFactory(['Ad','Ac','Ab','Aa','2b'])
    card2 = CardsModelFactory(['Jd','Jc','Jb','2d','2a'])
    print '%s'%card1
    print '%s'%card2
    print card1 > card2
    ...
    sf1 = ['Aa', '2a', '3a', '7a', '5a']
    sf2 = ['Ad', '2d', '3d', '6d', '5d']
    ins1 = CardsModelFactory(sf1)
    ins2 = CardsModelFactory(sf2)
    print('---------------------------------')
    print(ins2 > ins1)
    print(ins1 > ins2)
    #
    # cards = 'Ad,2d,3d,2c,3c,5c,5c,6c,4b,5b,6b,7b,8b'
    # cards = 'Ad,Ad,Ad,Ac,Ac,Ac,Ac,6c,4b,5b,6b,7b,8b'
